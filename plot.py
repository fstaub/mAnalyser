...
def HistPlot_Account(in_data, start, end, bar):
    ...
    return HistPlot(data, dates, labels, bar)
...
def HistPlot_Changes(in_data, start, end, w, h):
    ...
    return BarPlot(data, dates)    

def HistPlot_InOut(in_data, keywords, start, end,bar):
...
def HistPlot(data, dates, labels, bar):
    dim = len(data[0])
    w = 2.
    # dimw = w / dim
    dimw = 2
...
def BarPlot(data, dates):
    ...
    target, ax = plt.subplots()
    x = 3*np.arange(len(data))
    b = ax.bar(x, data, dimw, color='green')
...
def BarAverage_Accounts(in_data,keywords,start,end,w,h):
    # data to plot
    sum_all = analyse.summary_months(in_data,list(keywords.keys()))
    dates = analyse.find_dates(sum_all)
    dates = [d for d in dates if (
